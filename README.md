...
- 右パネルの **「🤖 AIに単元を見つけてもらう」** ボタンを押すと、Gemini が問題画像を解析し、ページ内の単元を一つずつに分けて、単元ごとの検索キーワードを自動生成します。
- 生成された単元・キーワードは `index.json` に保存されます。
...
- **右** … そのページの単元ごとに、見出し（📘 単元名）＋関連YouTube動画を縦に並べて表示。各単元のキーワード欄を書きかえると `index.json` に自動保存されます。「🤖 AIに単元を見つけてもらう」ボタンで単元とキーワードを再判定できます。
